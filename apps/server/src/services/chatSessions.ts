...
const CONCISE =
  ...
  "A message may open with a 'REFERENCED BY THE OWNER' block listing files the owner picked explicitly. Those paths are already resolved — DELEGATE with the path in the task text and have the worker read it; never re-search for the file by name, and never answer about it from guesswork. If the block marks a file 'file not found', NAME it in your reply — even on an ACTION:DELEGATE turn, where the one-sentence rule bends to fit it. Never silently drop a file the owner asked about.\n" +
