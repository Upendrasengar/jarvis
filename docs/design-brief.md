...
## Approved patterns (owner-reviewed — keep and extend these)

- **List selection (from the Calls sidebar)**: the ACTIVE item is a rounded
  card with a 1px accent border and a soft accent-tinted fill; INACTIVE
  items are borderless and quiet — title in medium-weight sans, one small
  mono meta line (time · source · state) beneath. Group headers are tracked
  mono caps (dates). Apply this same selection language to every list
  sidebar: Calls, Digest history, Notes, and any future list/detail pane.
  Selection is shown by presence (border + fill), never by inverting colors
  or bolding alone.
